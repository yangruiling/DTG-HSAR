...
class Graph():
    ...
    Args:
        ...
        max_hop (int): the maximal distance between two connected nodes  2个相邻节点之间的最大距离
        dilation (int): controls the spacing between the kernel points    控制kernel points之间的间距
    ...
    def get_adjacency(self, strategy):
        ...
        if strategy == 'uniform':    #统一标签
            A = np.zeros((1, self.num_node, self.num_node))
            A[0] = normalize_adjacency
            self.A = A
        elif strategy == 'distance':      #距离分割
            A = np.zeros((len(valid_hop), self.num_node, self.num_node))
            for i, hop in enumerate(valid_hop):
                A[i][self.hop_dis == hop] = normalize_adjacency[self.hop_dis ==
                                                                hop]
            self.A = A
        elif strategy == 'spatial':       #空间配置
            A = []
            for hop in valid_hop:
