...
def downsample(data_numpy, step, random_sample=True):
    # input: C,T,V,M  C:N个关键点表示的维度=3 (x,y,score)、T帧、点集V、边集E
    begin = np.random.randint(step) if random_sample else 0   #生成[0,step)范围内离散均匀分布的整数值
    return data_numpy[:, begin::step, :, :]

#numpy.random.rand(d0,d1,…dn)  以给定的形状创建一个数组，并在数组中加入在[0,1]之间均匀分布的随机样本
#numpy.random.rand(d0,d1,…dn)  以给定的形状创建一个数组，数组元素来符合标准正态分布N(0,1)
#numpy.random.randint(low,high=None,size=None,dtype)  生成在半开半闭区间[low,high)上离散均匀分布的整数值;若high=None，则取值区间变为[0,low)
#numpy.random.random_integers(low,high=None,size=None)  生成闭区间[low,high]上离散均匀分布的整数值;若high=None，则取值区间变为[1,low]
#numpy.random.random_sample(size=None)  以给定形状返回[0,1)之间的随机浮点数

#时间切片  张量:每一阶维数 = 该阶元素的数目    eg:最后一个数字表示数组a中最小单元中元素个数
#numpy.transport()  设原始维度对应轴0，1，2，3，4（eg：第一个方括号对应0轴）按照坐标轴顺序交换序列位置  
