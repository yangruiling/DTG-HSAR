...
def test(model_cfg,
    ...
    # 将终端输出的内容存入txt文件中，用于可视化
    Top1 = topk_accuracy(results, labels, 1)
    Top2 = topk_accuracy(results, labels, 5)
    doc = open('/home/yangruiling/mmskeleton/static/draw_data/ST_GCN.txt', 'w')
    data = [[(results.shape)],[(labels.shape)],[round(Top1, 4)],[round(Top2, 4)]]
    doc.write(str(data))
    doc.close()
...
def topk_accuracy(score, label, k=1):
    rank = score.argsort()     #argsort 是numpy中的一个函数 ，用来返回一个数组排好序后各元素对应的原来的位置序号
    doc = open('./sk.txt', 'a') 
    print(score,label,file=doc)  #将打印内容写入文件中
    #print(score.shape,label.shape)
    #enumerate() 函数用于将一个可遍历的数据对象(如列表、元组或字符串)组合为一个索引序列，同时列出数据和数据下标，一般用在 for 循环当中
    #rank[i, -k:]   返回一个数字在数字列表中的排位
